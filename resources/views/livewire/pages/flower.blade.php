...
new class extends Component
{
    ...
    public function store()
    {
        ...
        $validatedData['code'] = strtoupper($this->code);
        $flower = \App\Models\Flower::create($validatedData);
        $this->reset();
    ...
    {
        $this->editForm = true;
        $this->titleForm = 'Edit Flower';
        $this->actionForm = 'update';
        $this->user = \App\Models\Flower::where('id', $this->idUser)->first();
    ...
    public function update()
    {
        ...
        $validatedData['code'] = strtoupper($this->code);
        $flower = \App\Models\Flower::where('id', $this->idUser)->first();
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        @foreach($flowers as $flower)
                            <tr class="border-b dark:border-gray-700 hover:bg-gray-200">
                                <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $loop->index + $flowers->firstItem() }}</th>
                                <td class="px-4 py-3">{{ $flower->code }}</td>
                                <td class="px-4 py-3">{{ $flower->name }}</td>
                                <td class="px-4 py-3">{{ $flower->price }}</td>
                                <td class="px-4 py-3">{{ $flower->type }}</td>
                                <td class="px-4 py-3">{{ $flower->updated_at->format('d-m-Y H:i:s') }}</td>
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <button
                                        wire:click="getUserId({{ $flower->id }})"
                                        @click="$dispatch('openaction-usermodal{{ $flower->id }}')"
                                    ...
                                    <div
                                        x-data = "{ show : false }"
                                        x-show = "show"
                                        x-on:openaction-usermodal{{ $flower->id }}.window = "show = true"
