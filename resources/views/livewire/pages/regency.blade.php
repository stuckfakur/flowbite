...
new class extends Component
{
    ...
    {
        return [
            'regencies' => \App\Models\Regency::search($this->search)->orderBy('updated_at', 'desc')->Paginate(10)
        ];
    }
    public function getUserId($regencyId)
    {
        $this->idUser = $regencyId;
    }
    ...
    public function store()
    {
        ...
        $regency = \App\Models\Regency::create($validatedData);
        $this->reset();
    ...
    {
        $this->editForm = true;
        $this->titleForm = 'Edit Regency';
        $this->actionForm = 'update';
        $this->user = \App\Models\Regency::where('id', $this->idUser)->first();
    ...
    public function update()
    {
        ...
        $regency = \App\Models\Regency::where('id', $this->idUser)->first();
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        @foreach($regencies as $regency)
                            <tr class="border-b dark:border-gray-700 hover:bg-gray-200">
                                <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $loop->index + $regencies->firstItem() }}</th>
                                <td class="px-4 py-3">{{ $regency->name }}</td>
                                <td class="px-4 py-3">{{ $regency->city }}</td>
                                <td class="px-4 py-3">{{ $regency->updated_at->format('d-m-Y H:i:s') }}</td>
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <button
                                        wire:click="getUserId({{ $regency->id }})"
                                        @click="$dispatch('openaction-usermodal{{ $regency->id }}')"
                                    ...
                                    <div
                                        x-data = "{ show : false }"
                                        x-show = "show"
                                        x-on:openaction-usermodal{{ $regency->id }}.window = "show = true"
