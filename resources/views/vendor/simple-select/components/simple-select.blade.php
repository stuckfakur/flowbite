...
>
    <div
        ...
        {{ $attributes->class('block w-full bg-white border border-gray-300 rounded-md shadow-sm focus:ring-0 focus:ring-gray-400 focus:border-gray-400 sm:text-sm sm:leading-5')->only('class'); }}
    >
        <div x-show="!selected || selected.length === 0" class="flex flex-wrap">
            <div class="text-gray-800 rounded-sm w-full truncate py-1 my-0.5 flex flex-row items-center">
                <div class="w-full truncate text-gray-500 ps-2 dark:text-gray-500" x-text="placeholder">&nbsp;</div>
        ...
        @isset($attributes['multiple'])
            <div x-show="selected && typeof selected === 'object' && selected.length > 0" class="flex flex-wrap space-x-1">
                <template x-for="(value, index) in selected" :key="index">
                    <div class="text-gray-800 dark:text-gray-800 rounded-full truncate bg-gray-300 dark:bg-gray-800 py-1 my-0.5 flex flex-row items-center">
        ...
        @else
            <div x-show="selected" class="flex flex-wrap">
                <div class="text-gray-800 dark:text-gray-800 rounded-sm w-full truncate py-0.5 my-0.5 flex flex-row items-center">
                    ...
                    <div class="w-full py-1 ps-2 truncate">
                        @isset($customSelected)
    ...
    <div x-ref="simpleSelectOptionsContainer" x-bind:style="open ? 'height: ' + popperHeight : ''" class="absolute z-50 w-full">
        <div x-show="open">
            <input
                type="search"
                x-show="searchable"
                ...
                class="block w-full px-2 p-2 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 dark:text-gray-200 shadow-md focus:border-gray-200 focus:ring-0 sm:text-sm sm:leading-5"
            />
            <ul
                x-ref="simpleSelectOptionsList"
                class="absolute z-50 w-full py-1 overflow-auto text-base bg-white dark:bg-gray-600 shadow-lg rounded-b-md max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm"
                tabindex="-1"
                role="listbox"
            >
